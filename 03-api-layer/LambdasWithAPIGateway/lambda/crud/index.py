...
def format_sql(sql, params=None):
    """Format a SQL query with its parameters for preview display."""
    sql = ' '.join(sql.split())
    if not params:
        return sql
    formatted_params = []
...
def explain_query(cursor, sql, params=None):
    """Run EXPLAIN ANALYZE on a query and return the plan as a string."""
    cursor.execute(f'EXPLAIN ANALYZE {sql}', params or ())
    return '\n'.join(row[0] for row in cursor.fetchall())


def build_preview(cursor, steps):
    """Run EXPLAIN ANALYZE for each step that has SQL, inside a rolled-back transaction.
    Returns the response with steps enriched with explain output.
    """
    for step in steps:
    ...
    # Clean internal fields before sending response
    for step in steps:
...
def make_step(label, description, sql=None, params=None):
    """Create a step dict. If sql/params provided, formats SQL for display and keeps raw for EXPLAIN."""
...
def handler(event, context):
    ...
    # resource looks like /users, /users/{id}, or /accounts/{id}/balance
    parts = resource.strip('/').split('/')
    entity = parts[0] if parts else ''
    has_id = len(parts) > 1
    sub_resource = parts[2] if len(parts) > 2 else None
