...
###############################################################################
# Enable verbose logging for control plane events (master) while keeping
# tserver logging minimal to avoid excessive CloudWatch costs.
###############################################################################
echo "Enabling verbose logging..."
echo "--v=2" >> "${YB_HOME}/master/conf/server.conf"
echo "--v=0" >> "${YB_HOME}/tserver/conf/server.conf"
echo "--vmodule=raft_consensus=2,consensus_queue=2,tablet_peer=2,catalog_manager=2,tablet_split_manager=2,async_initializer=1,load_balancer=2" >> "${YB_HOME}/master/conf/server.conf"
echo "--vmodule=raft_consensus=1,tablet_service=1" >> "${YB_HOME}/tserver/conf/server.conf"
echo "--ysql_log_statement=all" >> "${YB_HOME}/tserver/conf/server.conf"
